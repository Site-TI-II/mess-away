        ...
        try {
            ...
            try (Connection conn = Database.connect()) {
                String sql = """
                    ...
                    JOIN USUARIO u ON t.id_usuario_responsavel = u.id_usuario
                    JOIN CATEGORIA cat ON t.id_categoria = cat.id_categoria
                    WHERE c.id_casa = ? AND c.ativo = true
                ...
                try (PreparedStatement stmt = conn.prepareStatement(sql)) {
                    ...
                    while (rs.next()) {
                        ...
                        tarefa.put("idTarefa", rs.getLong("idTarefa"));
        ...
        try {
            ...
            try (Connection conn = Database.connect()) {
                ...
                try (PreparedStatement stmt = conn.prepareStatement(sql)) {
                    ...
                    stmt.setInt(7, frequencia);
                    
                    ResultSet rs = stmt.executeQuery();
                    if (rs.next()) {
                        long idTarefa = rs.getLong("id_tarefa");
                        Map<String, Object> result = new HashMap<>();
                        result.put("idTarefa", idTarefa);
